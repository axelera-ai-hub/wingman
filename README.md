...
Current release tag: `v1.7.28`

## Assets

- Linux Debian packages: `wingman_1.7.28_amd64.deb`,
  `wingman_1.7.28_arm64.deb`
...
Download the assets from the GitHub Release for `v1.7.28`.
...
(
  ...
  test "$(sha256sum install-wingman.sh | awk '{ print tolower($1) }')" \
    = "$installer_hash"
  bash ./install-wingman.sh --version 1.7.28
)
```
